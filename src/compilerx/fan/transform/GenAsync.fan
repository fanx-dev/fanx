...
//////////////////////////////////////////////////////////////////////////
  ...
  private Void genCtx()
  {
    ...
    compiler.pod.addTypeDef(ctxCls)

    addField(curType.asRef, "self")

    curMethod.getAllVars.each |v| {
    ...
    if (!isStatic) {
      ...
      s := ExprStmt(
         BinaryExpr.makeAssign(
           FieldExpr.makeField(loc, ThisExpr.makeType(loc, curType.asRef), field, false),
  ...
  private Void genNext()
  {
    ...
    //self.foo$async(this)
    field := ctxCls.fieldDef("self")
    fieldE := FieldExpr.makeField(loc, ThisExpr.makeType(loc, curType.asRef), field, false)
    cs := CallExpr.makeWithMethod(loc, fieldE, implMethod, [ThisExpr.makeType(loc, ctxCls.asRef)])
    m.code.add(cs.toStmt)
    
    //return this.state != -1
    field2 := asyncCls.field("state")
    field2E := FieldExpr.makeField(loc, ThisExpr.makeType(loc, curType.asRef), field2, false)
  ...
  private Void genAsyncMethod()
  {
    ...
    doCall.addParamVar(ctxCls.asRef, "\$ctx")
  ...
  private Void genInternalMethod()
  {
    ...
    Expr[]? args := Expr[,]
    if (!isStatic) {
      args.add(ThisExpr.makeType(loc, curType.asRef))
    }

    if (doCall.paramDefs.size > 0) {
      doCall.paramDefs.each |param| {
        var_v := param;//MethodVar.makeForParam(doCall, doCall.vars.size+1, param, param.paramType)
  ...
  private Void genExportMethod() {
    ...
    if (curMethod.paramDefs.size > 0) {
      args = Expr[,]
      curMethod.paramDefs.each |param| {
        var_v := param;//MethodVar.makeForParam(curMethod, curMethod.vars.size+1, param, param.paramType)
    ...
    ctx := CallExpr.makeWithMethod(loc, ThisExpr.makeType(loc, curType.asRef), internalMethod, args)
...
//////////////////////////////////////////////////////////////////////////
  ...
  private Expr fieldExpr(Loc loc, Str name) {
    ...
    return FieldExpr.makeField(loc, LocalVarExpr(loc, implMethod.paramDefs.first), field, false)
  }
...
//////////////////////////////////////////////////////////////////////////
    ...
    implMethod.code.stmts.each |stmt| {
      if (stmt.id === StmtId.expr) {
        ...
        setState := BinaryExpr.makeAssign(fieldExpr(stmt.loc, "state")
            , Expr.makeForLiteral(stmt.loc, -1))
  ...
  private Void genCheckErr(Stmt[] stmts, Loc loc) {
    ...
    //tmpErr = ctx.err
    var_v := implMethod.addLocalVar(loc, ns.errType, "tmpErr", null)
    lvar := LocalVarExpr(loc, var_v)
    storeTmp := BinaryExpr.makeAssign(lvar, err)
    stmts.add(storeTmp.toStmt)

    ///ctx.err = null
    store := BinaryExpr.makeAssign(err, LiteralExpr.makeNull(loc))
    stmts.add(store.toStmt)
  ...
  private Void genYield(Expr c, TargetLabel breakLabel, SwitchTable table, Stmt[] stmts) {
    ...
    setState := BinaryExpr.makeAssign(fieldExpr(c.loc, "state")
        , Expr.makeForLiteral(c.loc, table.jumps.size))
    stmts.add(setState.toStmt)

    //ctx.waitFor(foo_())
    arg := TypeCheckExpr.coerce(c, ns.objType.toNullable)
    ctx := LocalVarExpr(loc, implMethod.paramDefs.first)
