...
//////////////////////////////////////////////////////////////////////////
  ...
  new make(CompilerContext compiler, Loc loc, FPod fpod, MethodDef? curMethod)
  {
  ...
  Void block(Block block)
  {
    if (block.stmts.last?.id === StmtId.targetLable) {
      ...
      if (curMethod != null && curMethod.ret.isVoid) {
        ...
        block.stmts.add(ThrowStmt(loc, LiteralExpr.makeNull(loc)))
      }
    }
  ...
  private Void localVarDefStmt(LocalDefStmt stmt)
  {
    if (stmt.isCatchVar)
    {
      ...
      if (var_v.isWrapped)
      {
        ...
        callNew(var_v.wrapField.parent.asRef.method("make"), 1)
      }
  ...
  private Void call(CallExpr call, Bool leave := call.leave)
  {
    ...
    if (call.isSafe)
    {
      ...
      if (target.ctype.isVal || method.parent.isVal)
        coerceOp(target.ctype, call.method.parent.asRef)
    }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  private Void line(Loc loc)
  {
    line   := loc.line
    offset := code.size
    if (line == -1 || lastLine == line || lastOffset == offset) return
    lineCount++
