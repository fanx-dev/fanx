...
using compilerx
using compilerx::IncCompiler as FanCompiler
...
//////////////////////////////////////////////////////////////////////////
  ...
  Str compile(Str text)
  {
    ...
    {
      buf := StrBuf()
      compiler.context.log.errs.each |err| { buf.add("$err.loc.line:$err.loc.col:$err.msg\n") }
      echo(buf)
      Env.cur.exit(-1)
    }

    this.dependencies = astpod.depends.map { Pod.find(it.name) }
