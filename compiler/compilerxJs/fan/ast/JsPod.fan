...
class JsPod : JsNode
{
  new make(JsCompilerSupport s, PodDef pod, TypeDef[] defs) : super(s)
  {
    ...
    s.compiler.input.jsFiles?.each |f| { natives[f.name] = f }
    jsOutput := s.compiler.input.onlyJs
    ...
    if (baseDir != null)
    {
      s.compiler.input.resFiles?.each |file|
      {
  ...
  override Void write(JsWriter out)
  {
    ...
    if (this.typeof.pod.config("sourceMapping") == "true")
