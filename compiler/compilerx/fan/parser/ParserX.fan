...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ...
  private Bool recoverToSlotDef() {
    while (curt != Token.eof) {
      if (curt == Token.constKeyword || curt == Token.funKeyword || curt == Token.varKeyword || curt == Token.letKeyword || curt == Token.newKeyword) {
        return true
      }
      if (curt == Token.classKeyword || curt == Token.mixinKeyword) {
        return false
...
//        }
    ...
    if (curt === Token.varKeyword || curt === Token.letKeyword || tokens[pos-1].kind === Token.constKeyword) {
      ...
      if (curt === Token.identifier && peekt === Token.colon) {
        ...
        err("expected 'name: Type' for field def", loc)
      }
    }
    ...
    throw err("expected slot define: func var let const new", loc)
  }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  private FieldDef fieldDef(Loc loc, TypeDef parent, DocDef? doc, FacetDef[]? facets, Int flags, CType? type, Str name)
  {
    ...
    {
      if (curt === Token.defAssign) err("Must use = for field initialization")
      consume
      curSlot = field
      inFieldInit = true
      field.init = expr
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  ...
  private ParamDef paramDef()
  {
    ...
    else throw err("expected ':' for param type");
    
    if (curt === Token.defAssign || curt === Token.assign)
    {
      if (curt === Token.defAssign) err("Must use = for parameter default");
      consume
      param.def = expr
    }
    endLoc(param)
  ...
  **
  ...
  private Bool funcTypeFormal(Bool isTypeRef, CType[] params, Str[] names, Bool[] unnamed)
  {
    if (isTypeRef) {
      ...
      err("expected name or type", cur.loc)
      consume
      return false
    }
  }
