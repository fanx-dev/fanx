...
//////////////////////////////////////////////////////////////////////////
  ...
  override Void run()
  {
    ...
    compiler.pod.types.each |TypeDef t| { scanType(t) }

    // process all the closures themselves
    compiler.pod.closures.each |c| { processClosure(c) }
  }
  ...
  **
  private Bool wrapNonFinalVars(MethodDef m)
  {
    ...
    m.getAllVars.each |var_v, i|
    {
      ...
      wrapField := genWrapper(compiler, var_v.ctype)
      ...
      if (var_v.isParam)
      {
        ...
        w := m.addLocalVar(var_v.loc, wrapField.parent.asRef, var_v.name + "\$Wrapper", m.code)
      ...
      else
      {
        ...
        var_v.ctype = wrapField.parent.asRef
      }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  private Stmt[]? fixLocalDef(LocalDefStmt stmt)
  {
    ...
    Expr? init
    if (stmt.init == null)
      init = LiteralExpr.makeNull(stmt.loc)
    else
  ...
  **
  ...
  {
    wrapCtor := var_v.wrapField.parent.methodDef("make")
...
//////////////////////////////////////////////////////////////////////////
  ...
  private Void optimizeNoCaptureVar(ClosureExpr closure) {
    ...
    field.fieldType = implType.asRef
    ...
    assign := BinaryExpr.makeAssign(FieldExpr.makeField(loc, StaticTargetExpr(loc, implType.asRef), field, false), nins)
    staticInit.code.add(assign.toStmt)
    staticInit.code.add(ReturnStmt.makeSynthetic(loc))

    target := StaticTargetExpr(loc, implType.asRef)
    closure.substitute = FieldExpr.makeField(loc, target, field, false)
  }
  ...
  **
  private Void processClosure(ClosureExpr closure)
  {
    ...
    shadowed := closure.doCall.getAllVars.findAll |var_v| {
        var_v.shadows != null
    }

    // process each shadowed variable
    shadowed.each |var_v, i| { 
        addVarToClosure(closure, var_v, var_v.name+"\$"+i)
    }
  ...
  **
  static CField makeOuterThisField(ClosureExpr closure)
  {
    ...
    else
    {
      ...
      subArg = ThisExpr(loc, closure.enclosingType.asRef)
    }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  static CField genWrapper(CompilerContext cs, CType ctype)
  {
    // build class name key
    suffix := ctype.isNullable ? "\$n" : ""
    podName := ctype.podName != "sys" ? "\$" + toSafe(ctype.podName) : ""
    name := "Wrap" + podName + "\$" + toSafe(ctype.name) + suffix

    // reuse existing wrapper
    existing := cs.wrappers[name]
    if (existing != null) return existing

    // define new wrapper
    loc := Loc("synthetic")
    w := TypeDef(loc, cs.syntheticsUnit, name)
    w.flags = FConst.Internal + FConst.Synthetic
    w.setBase(cs.ns.objType)
    cs.pod.addTypeDef(w)
    ...
    pvar  := param;//MethodVar.makeForParam(ctor, 1, param, param.paramType)
    ctor.params.add(param)
    //ctor.code.vars.add(pvar)
    ctor.code   = Block(loc)
    lhs := fieldExpr(loc, ThisExpr.makeType(loc, w.asRef), f)
...
//////////////////////////////////////////////////////////////////////////
  ...
  private static FieldExpr fieldExpr(Loc loc, Expr target, CField field)
  {
    ...
    FieldExpr.makeField(loc, target, field, false)
  }
